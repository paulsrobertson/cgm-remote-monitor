...
function init (env, cb, forceNewConnection) {
  ...
  function maybe_connect (cb) {
    ...
    if (connection != null && !forceNewConnection) {
      ...
      if (!env.storageURI) {
        throw new Error('MongoDB connection string is missing. Please set DB_URI environment variable');
      }
          ...
          .then(client => {
            ...
            .catch(err => {
              if (err.message && err.message.includes('AuthenticationFailed')) {
                ...
                cb(new Error('MongoDB authentication failed! Double check the URL has the right username and password in DB_URI.'), null);
                return;
              }
              ...
              if (err.name && err.name === "MongoNetworkError") {
                ...
                if (i == 1) cb(new Error('MongoDB connection failed! Double check the DB_URI setting in Heroku.'), null);
              } else {
                cb(new Error('DB_URI ' + env.storageURI + ' seems invalid: ' + err.message));
              }
            });
